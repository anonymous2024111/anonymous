...
# DGL
with open('./eva100/end2end/gcn_no_pre/result/dgl-v3.csv', 'r') as file:
...
#TC-GNN
# with open('./eva100/end2end/gcn_no_pre/result/tcgnn-v3.csv', 'r') as file:
#     reader = csv.reader(file, delimiter=',')
#     next(reader)  # 跳过第一行
#     for cur, row in enumerate(reader, start=0):
...
# Advisor
with open('./eva100/end2end/gcn_no_pre/result/advisor-v3.csv', 'r') as file:
    reader = csv.reader(file, delimiter=',')
    next(reader)  # 跳过第一行
    for cur, row in enumerate(reader, start=0):
        speedup['layer-hidden'].append(row[1])
...
# #PYG
# with open('./eva100/end2end/gcn_no_pre/result/pyg-v3.csv', 'r') as file:
#     reader = csv.reader(file, delimiter=',')
#     next(reader)  # 跳过第一行
#     for cur, row in enumerate(reader, start=0):
#         speedup['layer-hidden'].append(row[1])
...
# MGCNtf32
with open('./eva100/end2end/gcn_no_pre/result/mgcn32-v3.csv', 'r') as file:
    reader = csv.reader(file, delimiter=',')
    next(reader)  # 跳过第一行
    for cur, row in enumerate(reader, start=0):
        speedup['layer-hidden'].append(row[1])
...
# MGCNfp16
with open('./eva100/end2end/gcn_no_pre/result/mgcn16-v3.csv', 'r') as file:
    reader = csv.reader(file, delimiter=',')
    next(reader)  # 跳过第一行
    for cur, row in enumerate(reader, start=0):
        speedup['layer-hidden'].append(row[1])
...
# 显示图形
plt.savefig('./eva100/end2end/gcn_no_pre/result/gcn_end_800.png', dpi=800)
# 清空图形
plt.clf()
