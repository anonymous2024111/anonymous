...
def kernel_tf32_v2(inputInfo, epoches):
    ...
    X_prime, spmm_ms_avg =  MagicsphereGAT_kernel.forward_gen_tf32(inputInfo.x.size(1), inputInfo.num_nodes, inputInfo.row_pointers, inputInfo.column_index, inputInfo.degrees, 
                            inputInfo.x, inputInfo.max, epoches, 8)
...
def kernel_fp16_v2(inputInfo, epoches):
    ...
    X_prime, spmm_ms_avg =  MagicsphereGAT_kernel.forward_gen(inputInfo.x.size(1), inputInfo.num_nodes, inputInfo.row_pointers, inputInfo.column_index, inputInfo.degrees, 
                               inputInfo.x, inputInfo.max, epoches, 8)
    return round(spmm_ms_avg.item(),4)
