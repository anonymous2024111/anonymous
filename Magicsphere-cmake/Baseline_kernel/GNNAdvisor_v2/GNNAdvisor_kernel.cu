...
__global__ void SAG_cuda_kernel(
    ...
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> input,
    torch::PackedTensorAccessor64<int,1,torch::RestrictPtrTraits> row_pointers, 
    torch::PackedTensorAccessor64<int,1,torch::RestrictPtrTraits> column_index,
    torch::PackedTensorAccessor64<float,1,torch::RestrictPtrTraits> degrees,
    torch::PackedTensorAccessor64<int,1,torch::RestrictPtrTraits> part_pointers,
    torch::PackedTensorAccessor64<int,1,torch::RestrictPtrTraits> part2Node,
    const int num_nodes, 
    const int dim,
    const int num_parts,
    const int partSize,
...
__global__ void spmm_backward_cuda_kernel(
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> d_input,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> d_output,
    torch::PackedTensorAccessor64<int,1,torch::RestrictPtrTraits> row_pointers,
    torch::PackedTensorAccessor64<int,1,torch::RestrictPtrTraits> column_index,
    torch::PackedTensorAccessor64<float,1,torch::RestrictPtrTraits> degrees,
    torch::PackedTensorAccessor64<int,1,torch::RestrictPtrTraits> part_pointers,
...
torch::Tensor SAG_cuda(
    ...
    AT_DISPATCH_FLOATING_TYPES(input.type(), "Scatter_and_Gather", ([&] {
                                SAG_cuda_kernel<scalar_t><<<grid, block, shared_memory>>>(
                                    ...
                                    input.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
                                    row_pointers.packed_accessor64<int,1,torch::RestrictPtrTraits>(), 
                                    column_index.packed_accessor64<int,1,torch::RestrictPtrTraits>(),
                                    degrees.packed_accessor64<float,1,torch::RestrictPtrTraits>(),
                                    part_pointers.packed_accessor64<int,1,torch::RestrictPtrTraits>(), 
                                    part2Node.packed_accessor64<int,1,torch::RestrictPtrTraits>(),
                                    num_nodes, 
                                    dim,
                                    num_parts,
                                    partSize,
...
) 
{
    ...
    AT_DISPATCH_FLOATING_TYPES(input.type(), "spmm_cuda_forward", ([&] {
                                spmm_forward_cuda_kernel<scalar_t><<<grid, block, shared_memory>>>(
                                    output.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
                                    tmp.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
                                    row_pointers.packed_accessor64<int,1,torch::RestrictPtrTraits>(), 
                                    column_index.packed_accessor64<int,1,torch::RestrictPtrTraits>(),
                                    degrees.packed_accessor64<float,1,torch::RestrictPtrTraits>(),
                                    part_pointers.packed_accessor64<int,1,torch::RestrictPtrTraits>(), 
...
std::vector<torch::Tensor> spmm_backward_cuda(
    ...
    AT_DISPATCH_FLOATING_TYPES(d_output.type(), "spmm_cuda_backward", ([&] {
                                spmm_backward_cuda_kernel<scalar_t><<<grid, block, shared_memory>>>(
                                    d_input_prime.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
                                    d_output.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
                                    row_pointers.packed_accessor64<int,1,torch::RestrictPtrTraits>(),
                                    column_index.packed_accessor64<int,1,torch::RestrictPtrTraits>(),
                                    degrees.packed_accessor64<float,1,torch::RestrictPtrTraits>(),
                                    part_pointers.packed_accessor64<int,1,torch::RestrictPtrTraits>(), 
...
std::vector<torch::Tensor> spmm_backward_cuda_gin(
    ...
    AT_DISPATCH_FLOATING_TYPES(d_output.type(), "spmm_cuda_backward_gin", ([&] {
                                spmm_backward_cuda_kernel_gin<scalar_t><<<grid, block, shared_memory>>>(
                                    d_input.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
                                    d_input_prime.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
                                    row_pointers.packed_accessor64<int,1,torch::RestrictPtrTraits>(),
                                    column_index.packed_accessor64<int,1,torch::RestrictPtrTraits>(),
                                    epsilon,
