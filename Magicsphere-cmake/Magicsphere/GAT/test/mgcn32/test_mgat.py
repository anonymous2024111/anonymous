...
def kernel(inputInfo, epoches, nOri, mOri):
        att =  MagicsphereGAT_cmake.tf32_sddmm_csr(inputInfo.x.size(1), inputInfo.num_nodes, inputInfo.row_pointers, inputInfo.column_index, inputInfo.degrees, 
                               inputInfo.x, inputInfo.x, inputInfo.max, inputInfo.num_edges)

        output=MagicsphereGCN_cmake.forward_tf32_v2_csr(inputInfo.row_pointers, inputInfo.column_index, inputInfo.templete, att,  inputInfo.x, inputInfo.num_nodes, inputInfo.x.size(1), inputInfo.num_nodes_ori)
        print()
