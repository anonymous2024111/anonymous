...
class MGCN_dataset(torch.nn.Module):
    ...
    def init_edges(self):
        ...
        scipy_coo = coo_matrix((val, self.m_edge_index_new), shape=(self.num_nodes, self.num_nodes))
        adj = scipy_coo.tocsr()
        ...
        dd=torch.tensor(dd, dtype=torch.float16) 
        # self.dd= torch.rsqrt(dd)  
        self.dd= torch.ones_like(dd)
        self.row_pointers, self.column_index, self.degrees, self.templete=MagicsphereBlock_cmake.blockProcess8_16_csr(self.row_pointers1, self.column_index1)
    ...
    def init_embedding(self, dimN):
        ...
        '''
        # 打印归一化后的特征
        self.x = torch.randint(low=1, high=3, size=(self.num_nodes_ori, dimN) ,dtype=torch.float16)
    ...
    def to(self, device):
        ...
        self.templete = self.templete.to(device) 
